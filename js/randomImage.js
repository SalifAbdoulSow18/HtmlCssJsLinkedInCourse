...
// appliquer les images comme background de pageContent
pageContent.style.backgroundImage = `url(images/${randomImages.filename})`;
...
if (pictInfo) {
    ...
    // creer le title de pictInfo et l'ajouter dans le div
    let pictTitle = document.createElement('h1');
    let pictTitleContent = document.createTextNode(randomImages.name);
    pictTitle.appendChild(pictTitleContent);
    pictInfo.appendChild(pictTitle)

    // creer les P de l'auteur et description du pictInfo et les ajouter dans le div
    let pictAuthor = document.createElement('p');
    let pictData = document.createElement('p');
    let pictAuthorContent = document.createTextNode(randomImages.author);
    let pictDataContent = document.createTextNode(randomImages.exposure);
